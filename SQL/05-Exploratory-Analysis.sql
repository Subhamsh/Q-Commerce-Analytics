...
select 
	user_Type, 
	round(sum(final_amount_rupees)/count(order_id),2) as AOV
from orders 
where order_status = 'Delivered'
group by user_type;
...
select 
	user_type, 
	round(avg(customer_lifetime_orders),2) as avg_lifetime_orders
from orders
group by user_type;
...
select 
	dark_store_id, count(order_id) as total_orders, 
	count(order_id) filter(where order_status = 'Cancelled' or order_status  = 'Returned') as cancel_return_count, 
	round(
		(count(order_id)
			filter(
				where order_status = 'Cancelled' or order_status  = 'Returned'
				) * 100):: numeric / count(order_id),
...
-- Average Delivery Time of Delivered orders- 40.55
...
from orders
where 
	delivery_time_minutes is not null and 
...
select 
	category, 
	round(avg(discount_rupees),2) as avg_discount
from orders 
where discount_rupees > 0
group by category;
...
select round(sum(final_amount_rupees) / count(order_id),2) as AOV
from orders 
where discount_rupees > 0 and order_Status = 'Delivered';
...
with avg_orders_weekday as
(
	select count(order_id)/5 as avg_weekday_orders
	from orders 
	where is_weekend = 0
),
avg_orders_weekend as 
(
select count(order_id)/5 as avg_weekend_orders
from orders 
where is_weekend = 1 
)
select * from avg_orders_weekday cross join avg_orders_weekend;
...
select 
	payment_mode, 
	count(order_id) AS total_orders, 
	round(sum(final_amount_rupees),2) as revenue
from orders
where order_status = 'Delivered'
